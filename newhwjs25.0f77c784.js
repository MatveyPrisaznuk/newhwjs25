...
for(let i = 0; i < formFields.length; i += 1)formFields[i].addEventListener("change", changeHandler);
function changeHandler() {
    localStorage.setItem(this.name, this.value);
}
function checkStorage() {
    for(let i = 0; i < formFields.length; i += 1)if (formFields[i].type !== "submit") {
        const saved = localStorage.getItem(formFields[i].name);
        if (saved !== null) formFields[i].value = saved;
    }
}
checkStorage();

},{}]},["7wZbQ","2R06K"], "2R06K", "parcelRequiref0d8", {})

//# sourceMappingURL=newhwjs25.0f77c784.js.map
